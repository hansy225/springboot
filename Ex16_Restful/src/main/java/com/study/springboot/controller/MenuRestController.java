...
public class MenuRestController {
	...
	public List<Menu> findByType(@PathVariable(name="type") Type type) {  // name=키값, Type 변수명
	...
	/*
		ResponseEntity : 결과 데이터와 HTTP 상태코드와 오류코드까지 직접 제어할 수 있는 클래스
		 ...
		 * ResponseEntity 사용법
		  ...
		  + 상태 코드는 HttpStatus에 정의된 값 이용
		  + 상태 코드 OK와 body를 한번에 사용
		     ResponseEntity.ok.body(menu)
		  + body가 없을 때는, build() 사용
	...
	public Menu findById(@PathVariable(name="id") Long id) {
		...
		// 없는 id를 넣어서 오류가 났을 때 500(서버측 오류)가 뜸
		// 사용자의 잘못인 것(400~ 번대의 오류가 나야함
	} 
	*/
