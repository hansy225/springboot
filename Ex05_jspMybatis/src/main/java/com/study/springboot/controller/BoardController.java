	...
	/*
		* 요청시 전달한 값(파라미터)를 받는 방법
		 1. HttpServletRequest를 이용한 방법
		 	: 메서드의 매개변수에 넣는 방법
		 	  ...
		 	  public String detailView(HttpServletRequest request) {
		 	 	 int bno =  request.getParameter("boardno");  // 자동 형변환
		 	 }
		 	...
		 	: 메서드 위에 어노테이션을 넣는 방법
		 	 - 변수에 저장할 때 : request.getParameter("키")
		 	  ...
		 	  public String detailView(@RequestParam(value="boardno") int bno
		 	  						   @RequestParam(value="writer", defaulrValue="홍길동") String user) {
		 	  					   
		 	  }
		 
		 3. 매개변수명으로 사용하는 방법
		 	: 매개변수명 name값(요청시 전달값의 키)과 동일하게 세팅해줘야 자동으로 같이 주입됨
		 	  ex)
		 	  @GetMapping("/detail")
		 ...
		 4. @ModelAttribute 어노테이션을 사용하는 방법
		 	: 주로 객체를 받을 때 사용
		 	 - 요청시 전달값의 키 값(name값)을 bean 클래스에 담고자하는 필드명으로 작성
		 	 ** 반드시 name과 담고자하는 필드명이 동일해야 됨
		 	 ex)
		 	 @GetMapping("/detail")
